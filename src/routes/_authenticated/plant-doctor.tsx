...
function PlantTypeSelector({
  ...
  const allPlantOptions = Array.from(new Set([...PLANT_TYPES, ...userPlants])).sort((a, b) =>
    a.localeCompare(b),
  );
  const filteredOptions = allPlantOptions.filter((plant) =>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
      >
        <span className="flex items-center gap-2">
          {displayValue ? (
            <>
              <span>{getPlantEmoji(displayValue)}</span>
      ...
      >
        <span className="flex items-center gap-2">
          {value ? (
            ...
            <span className="text-muted-foreground">— Pilih bagian tanaman —</span>
          )}
        </span>
          ...
          <div className="py-1">
            {PLANT_PARTS.map((part) => (
              <button
                key={part.value}
                type="button"
                onClick={() => handleSelect(part.value)}
                className={cn(
                  "flex w-full items-center gap-2.5 px-3 py-2.5 text-sm transition-colors text-left hover:bg-muted/50",
                  value === part.value && "bg-primary/8 text-primary font-medium",
                )}
              >
...
function CameraModal({
  ...
  const attachAndPlay = useCallback(
    (video: HTMLVideoElement, stream: MediaStream, attemptId: number): Promise<void> => {
      return new Promise((resolve, reject) => {
        if (attemptIdRef.current !== attemptId) {
          reject(new DOMException("Stale attempt", "AbortError"));
          return;
        }
        ...
        const settle = () => {
          ...
          if (tracks.length === 0 || tracks[0].readyState !== "live") {
            reject(new DOMException("Track not live", "NotReadableError"));
            return;
          }
          resolve();
        };
  ...
  const buildConstraintCandidates = useCallback(
    (mode: "environment" | "user", isDesktop: boolean): MediaStreamConstraints[] => {
      if (isDesktop)
        return [
  ...
  const startCamera = useCallback(
    async (mode: "environment" | "user") => {
      ...
      if (!window.isSecureContext) {
        setErrorMsg(
          "❌ Kamera membutuhkan koneksi aman (HTTPS).\n\nJika di localhost, pastikan menggunakan http://localhost (bukan IP address).",
        );
        setStatus("error");
        return;
      }
      if (!navigator.mediaDevices?.getUserMedia) {
        setErrorMsg(
          "❌ Browser Anda tidak mendukung akses kamera.\n\nCoba Chrome, Firefox, atau Safari versi terbaru. Atau gunakan tombol Galeri.",
        );
        setStatus("error");
        return;
      }
      if (hasMultipleCamerasRef.current === null) {
        const count = await detectCameraCount();
        hasMultipleCamerasRef.current = count > 1;
      }
      ...
      try {
        await attachAndPlay(video, stream, myAttemptId);
        if (attemptIdRef.current !== myAttemptId) return;
        setStatus("ready");
      } catch (err: any) {
        if (err?.name === "AbortError") return;
        console.error("[Camera] attachAndPlay failed:", err);
        stopStream();
        if (attemptIdRef.current === myAttemptId) {
          setErrorMsg(
            "❌ Video tidak dapat ditampilkan.\n\nRefresh halaman dan coba lagi, atau gunakan tombol Galeri.",
          );
          setStatus("error");
        }
      }
  ...
  const flipCamera = useCallback(async () => {
    if (isFlipping || status === "initializing") return;
    setIsFlipping(true);
    const next = facingMode === "environment" ? "user" : "environment";
    setFacingMode(next);
    await startCamera(next);
    setIsFlipping(false);
  }, [isFlipping, status, facingMode, startCamera]);
          ...
          <>
            ...
            <div className="absolute bottom-0 inset-x-0 flex items-center justify-center gap-6 p-5 bg-gradient-to-t from-black/70 to-transparent">
              <button
                ...
                className="rounded-full bg-white/20 p-3 text-white hover:bg-white/30 disabled:opacity-50"
              >
                <SwitchCamera className={cn("h-5 w-5", isFlipping && "animate-spin")} />
              </button>
              <button
                onClick={capture}
                disabled={!isReady}
                className="h-16 w-16 rounded-full border-4 border-white bg-white/20 hover:bg-white/30 disabled:opacity-50 active:scale-95 flex items-center justify-center"
              >
    ...
    >
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1.5">
          <span className={cn("text-xs font-semibold", severityInfo.text)}>
            {severityInfo.label}
          </span>
        ...
        <div className="h-1.5 rounded-full bg-black/10 overflow-hidden">
          <div
            className={cn("h-full rounded-full transition-all duration-500", severityInfo.bar)}
            style={{ width: `${score}%` }}
          />
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {open && <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">{detail}</p>}
      </div>
      <ChevronRight
        className={cn(
        ...
        <div className="mt-3 flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap">
            <span
              className={cn(
                ...
                result.confidence >= 75 ? "text-success" : "text-warning",
              )}
            >
          ...
          <div
            className={cn(
              ...
              result.confidence >= 75 ? "bg-success/15 text-success" : "bg-warning/15 text-warning",
            )}
          >
        ...
        <div>
          ...
          <div className="rounded-xl border border-border overflow-hidden divide-y divide-border">
            <ActionRow
              ...
              label="Tindakan Awal (24-48 jam)"
              detail={result.initial_action}
            />
            <ActionRow
              icon={<span className="text-base">🛡️</span>}
              label="Penanganan Lengkap"
              detail={result.solution}
            />
            {result.fertilizer && (
              <ActionRow
                icon={<span className="text-base">💊</span>}
                label="Pupuk Dianjurkan"
                detail={result.fertilizer}
              />
            )}
            <ActionRow
              icon={<span className="text-base">🔍</span>}
              label="Tindak Lanjut"
              detail={result.follow_up}
            />
            {result.pesticide && (
              <ActionRow
                icon={<span className="text-base">💊</span>}
                label="Pestisida/Fungisida"
                detail={result.pesticide}
              />
            )}
            {result.weather_note && (
              <ActionRow
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground pt-1">
        ...
        <span>AI sedang menganalisa gambar tanaman Anda...</span>
      </div>
    </div>
  );
}
        ...
        {histLoading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 px-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Hasil diagnosa akan tersimpan otomatis di sini
            </p>
          </div>
        ) : (
          history.map((d) => {
...
function DiagnosaTanaman() {
  ...
  const userPlantNames = Array.from(
    new Set(plants.filter((p) => p.status === "Aktif").map((p) => p.name)),
  );
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const weatherSummary = weather
        ? `${weather.current.condition}, kelembapan ${weather.current.humidity}%`
        : weatherCondition;
      ...
      if (!diagnosis.is_plant_image)
        throw new Error(
          "Gambar yang diupload bukan foto tanaman. Harap upload foto daun, batang, atau buah tanaman yang ingin didiagnosa.",
        );
      const {
        data: { user },
      } = await supabase.auth.getUser();
...
if (insertError) {
  ...
  const deleteHistory = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("plant_diagnoses").delete().eq("id", id);
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Upload foto tanaman untuk analisa penyakit berbasis AI
          </p>
        </div>
              ...
              <div className="border-b border-border px-5 py-3.5 flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Upload foto yang jelas pada bagian tanaman yang bermasalah
                  </p>
                </div>
              </div>
              <div className="p-4 space-y-3">
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        JPG, PNG · Maks 5MB · Harus foto tanaman
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-semibold text-foreground">
                      Jenis Tanaman <span className="text-destructive">*</span>
                    </label>
                    <div className="mt-2">
                      <PlantTypeSelector
                        value={plantType}
                        ...
                        onCustomPlantChange={setCustomPlant}
                        userPlants={userPlantNames}
                      />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-foreground">
                      Bagian Tanaman <span className="text-destructive">*</span>
                    </label>
                    <div className="mt-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    💡 Membantu AI lebih akurat, terutama jika foto kurang jelas
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    {SOIL_CONDITIONS.map((s) => (
                      <button
                        key={s.value}
                        onClick={() => setSoilCondition(s.value)}
                        className={cn(
                          "flex flex-col items-center gap-1 rounded-xl border-2 p-3 text-center text-xs font-medium transition-all",
                          soilCondition === s.value
                ...
                {!preview && (
                  <p className="text-center text-xs text-muted-foreground -mt-2">
                    Upload foto tanaman terlebih dahulu
                  </p>
                )}
                {preview && !finalPlantLabel && (
                  <p className="text-center text-xs text-muted-foreground -mt-2">
                    Pilih jenis tanaman terlebih dahulu
                  </p>
                )}
                {preview && finalPlantLabel && !plantPart && (
                  <p className="text-center text-xs text-muted-foreground -mt-2">
                    Pilih bagian tanaman yang difoto terlebih dahulu
                  </p>
                )}
              ...
              <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-border bg-card/50 p-10 text-center min-h-[280px]">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Upload foto dan isi data tanaman, lalu klik "Mulai Diagnosa"
                  </p>
                </div>
