...
function Admin() {
  ...
  const { data: users = [], isLoading: usersLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("profiles")
        .select("id, full_name, location, created_at, avatar_url, phone, farmer_type, updated_at")
  ...
  const { data: reports = [], isLoading: reportsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [postsRes, profilesRes] = await Promise.all([
        postIds.length
          ? supabase.from("community_posts").select("id, title, user_id").in("id", postIds)
      ...
      (postsRes.data ?? []).forEach((p: any) => {
        postMap[p.id] = { title: p.title, user_id: p.user_id };
      });
      ...
      (authorProfiles ?? []).forEach((p: any) => {
        authorMap[p.id] = p.full_name ?? "Pengguna";
      });
      const reporterMap: Record<string, string> = {};
      (profilesRes.data ?? []).forEach((p: any) => {
        reporterMap[p.id] = p.full_name ?? "Pengguna";
      });
  ...
  const { data: diagLogs = [], isLoading: diagLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, user_id, plant_type, diagnosis, severity, confidence_score, created_at")
        .order("created_at", { ascending: false })
        .limit(100);
      if (!data?.length) return [];
      ...
      (profilesData ?? []).forEach((p: any) => {
        profileMap[p.id] = p.full_name?.trim() || "Tanpa Nama";
      });
  ...
  const flagPost = useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      ...
      const { data: post } = await supabase
        ...
        .eq("id", id)
        .maybeSingle();
      // Jangan kirim notif jika admin flag postnya sendiri
      if (post && user && post.user_id !== user.id) {
        await pushNotification(post.user_id, {
          ...
          body: JSON.stringify({
            ...
            reason: reason,
            action: "approved",
            message: `Postingan "${post.title}" ditandai oleh admin. Alasan: ${reason}.`,
          }),
          type: "warning",
        });
      }
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-posts"] });
      toast.success("Postingan ditandai");
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const unflagPost = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-posts"] });
      toast.success("Tanda peringatan dihapus");
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const rejectReport = useMutation({
    mutationFn: async (report: any) => {
      ...
      const notificationBody = JSON.stringify({
        ...
        action: "rejected",
        message: `Laporan Anda terhadap postingan "${report.post_title}" telah ditinjau dan tidak ditemukan pelanggaran.`,
      });
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-reports"] });
      qc.invalidateQueries({ queryKey: ["admin-stats"] });
      toast.success("Laporan ditolak & pelapor dinotifikasi");
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteUser = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-users"] });
      qc.invalidateQueries({ queryKey: ["admin-stats"] });
      toast.success("Akun pengguna dihapus");
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteDiagnoses = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["admin-stats"] });
      setSelectedDiagIds(new Set());
      toast.success("Data diagnosa dihapus");
    },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline" size="sm" className="gap-1.5">
            <Link to="/dashboard">
              <ExternalLink className="h-3.5 w-3.5" /> Buka App
            </Link>
          </Button>
          <Button
            variant="outline"
            ...
            {[
              {
                ...
                desc: "Blokir, aktifkan, atau hapus akun pengguna",
              },
              {
                tab: "moderation" as AdminTab,
                icon: Flag,
                label: "Moderasi Konten",
                desc: "Tandai atau hapus postingan komunitas",
              },
              {
                tab: "reports" as AdminTab,
                icon: AlertTriangle,
                label: "Laporan User",
                desc: `${stats?.reports ?? 0} laporan menunggu persetujuan`,
              },
            ].map((item) => (
              <button
                key={item.tab}
      ...
      {activeTab === "users" && (
        <div className="space-y-4">
                    ...
                    users.map((u: any) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          u.is_blocked && "opacity-60 bg-destructive/5",
                        )}
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground">
                          {u.created_at
                            ? format(parseISO(u.created_at), "d MMM yyyy", { locale: idLocale })
                            : "—"}
                        </td>
                        <td className="px-4 py-3">
                          {u.is_blocked ? (
          ...
          <p className="text-xs text-muted-foreground px-1">
            💡 Memblokir mencegah user login sementara. Menghapus permanen tidak bisa dipulihkan.
            Keduanya mengirim notifikasi ke user.
          </p>
        </div>
      )}
      ...
      {activeTab === "moderation" && (
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {(["all", "flagged"] as const).map((f) => (
                ...
                >
                  {f === "all"
                    ? "Semua Postingan"
                    : `⚠️ Ditandai (${posts.filter((p: any) => p.is_flagged).length})`}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-xl bg-muted/30 border border-border px-4 py-3 text-xs text-muted-foreground space-y-1">
            ...
            <p>
              🚩 <strong>Tandai</strong> — Beri peringatan ke pemilik. Tanda otomatis hilang saat
              pemilik memperbarui postingan.
            </p>
            <p>
              ✅ <strong>Hapus Tanda</strong> — Hapus status peringatan tanpa menghapus postingan.
            </p>
            <p>
              🗑️ <strong>Hapus</strong> — Hapus postingan permanen (tidak bisa dibatalkan).
            </p>
            <p>
              🔗 <strong>Lihat Postingan</strong> — Buka langsung di halaman komunitas.
            </p>
          </div>
      ...
      {activeTab === "reports" && (
        <div className="space-y-4">
          <div className="rounded-xl bg-muted/30 border border-border px-4 py-3 text-xs text-muted-foreground space-y-1">
            ...
            <p>
              2. Laporan masuk ke sini dengan status <strong>Menunggu</strong>.
            </p>
            <p>
              3. <strong>Setuju</strong> → postingan ditandai + pemilik mendapat notifikasi.
            </p>
            <p>
              4. <strong>Tolak</strong> → laporan ditutup tanpa aksi apapun.
            </p>
                  ...
                  <div className="flex items-start gap-3">
                    <AlertTriangle
                      className={cn(
                        ...
                        r.status === "pending" ? "text-amber-500" : "text-muted-foreground",
                      )}
                    />
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="rounded-lg bg-muted/50 p-3 space-y-1.5 text-xs text-muted-foreground mb-2">
                        <div className="flex gap-2">
                          <span className="w-28 shrink-0 font-semibold text-foreground/70">
                            Alasan
                          </span>
                        ...
                        {r.post_title && (
                          <div className="flex gap-2">
                            ...
                            <span>
                              "{r.post_title}" — oleh {r.post_author_name}
                            </span>
                          </div>
                        )}
                        <div className="flex gap-2">
          ...
          {selectedDiagIds.size > 0 && (
            <div className="flex items-center gap-3 rounded-xl bg-destructive/10 border border-destructive/20 px-4 py-2.5">
              <p className="text-sm font-semibold text-destructive flex-1">
                {selectedDiagIds.size} item dipilih
              </p>
              <Button
                size="sm"
                variant="outline"
                className="h-7 gap-1 px-3 text-xs"
                onClick={() => setSelectedDiagIds(new Set())}
              >
                Batal
              </Button>
              <Button
                  ...
                  <tr>
                    <th className="px-4 py-3">
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedDiagIds(
                            e.target.checked ? new Set(diagLogs.map((d: any) => d.id)) : new Set(),
                          )
                        }
                  ...
                  {diagLoading ? (
                    ...
                    diagLogs.map((d: any) => (
                      <tr
                        ...
                        className={cn(
                          "hover:bg-muted/30 transition-colors",
                          selectedDiagIds.has(d.id) && "bg-primary/5",
                        )}
                      >
                        <td className="px-4 py-3">
                          <input
