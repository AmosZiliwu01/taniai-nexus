...
function RichContent({
  ...
  if (!isHtml)
    return <p className={cn("text-sm leading-relaxed whitespace-pre-wrap", className)}>{html}</p>;
  return (
    <div
      className={cn(
        "text-sm leading-relaxed prose prose-sm max-w-none [&_ul]:list-disc [&_ul]:pl-4 [&_a]:text-primary [&_a]:underline",
        className,
      )}
      style={
        lineClamp
...
function Avatar({
  ...
  return (
    <div
      className={cn(
        "rounded-full bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center font-bold text-white shrink-0",
        sz,
      )}
    >
      {initials}
    ...
    <div className="rounded-xl border border-input overflow-hidden focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all">
      ...
      <div className="relative">
        ...
        {(!value || value === "<br>" || value === "") && (
          <div className="pointer-events-none absolute left-3 top-2.5 text-sm text-muted-foreground/50 select-none">
            {placeholder}
          </div>
        )}
      </div>
    ...
    <div className="space-y-1.5 pt-2">
      {replyTarget && (
        <div className="flex items-center gap-1.5 px-1">
          ...
          <span className="text-xs text-primary">
            Membalas <strong>@{replyTarget.name}</strong>
          </span>
          <button
            onClick={() => {
              onClearReply();
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 rounded-full border border-input bg-background px-3 py-1.5 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
          {replyTarget && (
            <span className="text-sm text-primary font-medium">@{replyTarget.name}</span>
          )}
          <input
            ref={inputRef}
            value={body}
    ...
    <div className={cn("flex gap-2", depth > 0 && "ml-8")}>
      <div className="flex flex-col items-center gap-0 shrink-0">
        ...
        {hasReplies && isExpanded && depth === 0 && (
          <div className="w-px flex-1 bg-border/60 mt-1 mb-0.5" />
        )}
      </div>
      <div className="flex-1 min-w-0 pb-1">
        ...
        <div className="flex items-center gap-3 mt-1 px-1 flex-wrap">
          ...
          <button
            ...
            className="text-[10px] font-semibold text-muted-foreground hover:text-primary flex items-center gap-0.5"
          >
          ...
          {hasReplies && depth === 0 && (
            <button
              ...
              className="ml-auto text-[10px] font-semibold text-primary flex items-center gap-0.5"
            >
              {isExpanded ? (
                <>
                ...
                <>
                  <ChevronDown className="h-3 w-3" /> {comment.replies.length} balasan
                </>
              )}
            </button>
          )}
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="flex w-full items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3.5 text-left hover:border-primary/30 shadow-card transition-all"
      >
        {currentUser ? (
          <Avatar name={currentUser.name} url={currentUser.avatar_url} size="sm" />
        ) : (
...
function TopDiscussionsWidget({ posts }: { posts: Post[] }) {
  const topPosts = useMemo(
    () => [...posts].sort((a, b) => b.comments_count - a.comments_count).slice(0, 5),
    [posts],
  );
  ...
  const { data: activeUsers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      (postsRes.data ?? []).forEach((r) => {
        scoreMap[r.user_id] = (scoreMap[r.user_id] ?? 0) + 3;
      });
      (commentsRes.data ?? []).forEach((r) => {
        scoreMap[r.user_id] = (scoreMap[r.user_id] ?? 0) + 2;
      });
...
function PostCard({
  ...
  const comments = useMemo((): Comment[] => {
    const withAuthors: RawComment[] = rawComments.map((c) => ({
      ...c,
      author: commentAuthors[c.user_id] ?? { full_name: null, avatar_url: null },
    }));
    ...
    return topLevel.map((top) => ({
      ...
      root_parent_id: null,
      replies: children
        .filter((c) => c.parent_id === top.id || getRootParent(c) === top.id)
        .sort((a, b) => a.created_at.localeCompare(b.created_at))
        .map((r) => ({ ...r, root_parent_id: top.id, replies: [] })),
    }));
    ...
    onSuccess: ({ newCount, liked }) => {
      qc.setQueriesData({ queryKey: ["community-posts"] }, (old: any) => {
        ...
        return old.map((p: Post) =>
          p.id === post.id ? { ...p, likes_count: newCount, liked_by_me: liked } : p,
        );
      });
    },
    onError: (e: Error) => toast.error(e.message),
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      toast.success(
        "Postingan diperbarui!" + (post.is_flagged ? " Tanda peringatan otomatis dihapus." : ""),
      );
    } catch (e: any) {
      toast.error(e.message);
    } finally {
  ...
  const reportPost = useMutation({
    mutationFn: async (reason: string) => {
      ...
      const { data: reporterRole } = await supabase
        .from("user_roles")
        .select("role")
        .eq("user_id", user.id)
        .eq("role", "admin")
        ...
        if (post.user_id !== user.id) {
          await supabase.from("notifications").insert({
            ...
            body: JSON.stringify({
              ...
              reason: reason,
              action: "approved",
              message: `Postingan "${post.title}" telah ditandai oleh admin. Alasan: ${reason}.`,
            }),
            type: "warning",
            is_read: false,
            wa_sent: false,
  ...
  const handleReply = useCallback(
    (target: { commentId: string; name: string; rootParentId: string | null }) => {
  ...
  return (
    <>
      ...
      >
                  ...
                  {post.is_flagged &&
                    (isOwn ? (
                      <span
                        ...
                        title={
                          post.flagged_reason
                            ? `Alasan: ${post.flagged_reason}`
                            : "Postingan ini sedang ditinjau oleh admin"
                        }
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <span className="rounded-full bg-destructive/10 px-1.5 py-0.5 text-[9px] font-semibold text-destructive border border-destructive/10">
                        Ditinjau
                      </span>
                    ))}
                </div>
            ...
            {post.image_url && (
              ...
              >
                <img
                  src={post.image_url}
                  alt=""
                  className="w-full h-full object-cover hover:opacity-90 rounded-xl border border-border/100"
                />
              </div>
            )}
            <div className="flex-1 min-w-0">
                ...
                <>
                  ...
                  {needsExpandButton && (
                    <button
                      ...
                      className="mt-1 text-xs text-primary/70 hover:text-primary font-medium"
                    >
        ...
        <div className="flex items-center border-t border-border/60 px-3 py-1.5">
          ...
          <button
            onClick={() => {
              setShowComments((v) => !v);
              if (showComments) setReplyTarget(null);
            }}
            className={cn(
              "flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium transition-colors",
              showComments
          ...
          <button
            onClick={handleShare}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium text-muted-foreground hover:bg-muted/50 hover:text-foreground"
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => reportPost.mutate(reportReason)}
        ...
        >
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {post.is_flagged && (
                <span className="text-xs rounded-full bg-warning/20 text-warning px-2 py-0.5 border border-warning/30">
                  ⚠️ Postingan ditandai — simpan untuk hapus tanda
                </span>
              )}
            </div>
            <div>
      ...
      {showFlaggedModal && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-2xl border border-border bg-card shadow-elevated p-5"
          ...
          >
            ...
            {post.flagged_reason ? (
              <>
                <p className="mt-1 text-sm text-muted-foreground">
                  Postinganmu ditandai oleh admin dengan alasan:
                </p>
                <p className="mt-2 text-sm font-semibold text-destructive">{post.flagged_reason}</p>
                <p className="mt-3 text-xs text-muted-foreground">
                  Perbarui konten postinganmu untuk menghapus tanda ini.
                </p>
              </>
            ) : (
              <p className="mt-1 text-sm text-muted-foreground">
                Postinganmu sedang ditinjau oleh tim moderasi kami.
              </p>
            )}
            <div className="mt-5">
              <Button className="w-full" onClick={() => setShowFlaggedModal(false)}>
                Mengerti
              </Button>
            </div>
          </div>
      ...
      {showDeletePostModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-card shadow-elevated p-5">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              <span className="font-medium text-foreground">"{post.title}"</span> akan dihapus
              permanen beserta semua komentarnya.
            </p>
            <div className="mt-5 flex gap-2">
              <Button
                variant="outline"
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Berbagi pengalaman & solusi bersama petani Indonesia
          </p>
        </div>
        <div className="relative">
